...
/*
 * GhostFibers — vendored from React Bits (https://reactbits.dev).
 *
 * Copyright (c) 2025 David Haz
 * Licensed MIT + Commons Clause. The Commons Clause permits use inside a
 * product like this site, but not selling or redistributing the component
 * itself. The notice above travels with the file to satisfy that.
 *
 * Kept as a faithful copy of upstream — the shader is the artwork — so a future
 * upstream fix can be diffed in. Our integration (theme colours, mount guards,
 * static fallback) deliberately lives outside it, in sections/HeroBackground.
 *
 * Local changes, kept minimal:
 *   • container styling moved from an imported .css file to a className, since
 *     this project has no global component stylesheets;
 *   • the colour-update effect re-reads on every render rather than tracking a
 *     26-item dependency array, which the React Compiler handles for us.
 */
...
export interface GhostFibersProps {
  lineColor?: string;
  glowColor?: string;
  /** Canvas backdrop. Must match the section behind it or a seam shows. */
  backdrop?: string;
  speed?: number;
  scale?: number;
  rotation?: number;
...
const GhostFibers: FC<GhostFibersProps> = ({
  ...
  /*
   * Upstream keys the live GL context off the container node in a module-level
   * WeakMap. Reaching the uniforms back through the container ref trips the
   * React Compiler's immutability rule (this project builds with
   * reactCompiler: true, upstream does not), so the context lives in its own
   * ref instead — which is also one fewer moving part.
   */
  const contextRef = useRef<GhostFibersContext | null>(null);
  ...
  useEffect(() => {
    ...
    // Recreating the GL context is only correct on a dpr change; every other
    // prop is pushed through uniforms by the effect below.
