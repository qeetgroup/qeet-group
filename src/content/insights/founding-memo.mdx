...
kind: perspective
topic: Perspectives
dek: Why we built Qeet Group as a technology organisation, what we mean by questions worth answering, and what we will not do.
...
## Why one organisation, and not several.

We could have built these as separate companies. The optics would have been more legible — each with its own story, its own funding, its own path. We chose not to, for one reason.

Almost every product we want to build needs the same two things underneath it: a way to know who someone is, and a way to look and behave consistently. Built separately, each product answers those questions again, slightly differently, and the differences compound into an organisation nobody can reason about as a whole. Access lingers after someone leaves. A pattern learned in one product does not transfer to the next. Every team spends part of its year rebuilding what already exists.

So the structure is the argument. One identity layer, one design foundation, one set of standards that does not bend per product — and above that, teams with real autonomy over their own architecture and roadmap. The connective layer is not a convenience the organisation offers its teams. It is the thing being built; the products are what it makes possible.

The test is not whether the products look alike. It is whether the next one costs less to build than the last one did.

We could also have built a single product and gone deep. We have done that before. We chose against it because the interesting problems here are the ones that only appear when software has to work together — and you cannot study those with one product.
...
That kind of compounding is mostly a structural property. It is hard to retrofit. We are designing the organisation to make it the default — by holding the quality bar above the launch bar, by giving teams autonomy with accountability, and by refusing to start work whose value peaks at launch.
